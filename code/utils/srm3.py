...
def to2dimension(spase_points, density_points):
    ...
    # 投影至平面，同时返回投影信息
    project_spase_points, projection_info_sparse ,project_spase2d= project_to_plane_with_info(spase_points, normal_vector)
    project_density_points, projection_info_dense ,project_density2d=  project_to_plane_with_info(density_points, normal_vector)
    ...
    return project_spase_points, project_density_points, projection_info,project_spase2d,project_density2d
...
def project_to_plane_with_info(points, normal_vector):
    ...
    I = np.eye(3)
    p = I - np.outer(normal_vector, normal_vector)

    project_3d = points @ p.T
    pca_2d = PCA(n_components=2)
    project_2d = pca_2d.fit_transform(project_3d)
    ...
    return points_2d, projection_info,project_2d
...
def project_to_plane(points, normal_vector):
    ...
    简化的投影函数（保持向后兼容）
    """
    points_2d, _, project2d = project_to_plane_with_info(points, normal_vector)
    return points_2d,project2d
    ...
    for i in range(windows_num):
        ...
        for j in range(windows_num):
            ...
            if  window_density < 1:
                ...
                current_windows_scale = [current_min_x, current_max_x, current_min_y, current_max_y]
                insert_indices = SPMREMapping(current_windows_scale, density_points_2d, alpha = 0.03)
...
def gradient_based_interpolation(current_window_scale, spase_points_2d, density_points_2d, alpha=0.7):
    ...
    try:
        ...
        # 在梯度方向进行插值
        num_gradient_steps = max(3, int((max_gradient_proj - min_gradient_proj) / 0.06))
        for i in range(num_gradient_steps):
            gradient_proj = min_gradient_proj + i * (max_gradient_proj - min_gradient_proj) / (num_gradient_steps - 1)

            # 在垂直梯度方向进行插值
            num_perp_steps = max(3, int((max_perp_proj - min_perp_proj) / 0.06))
...
def back_project_to_3d(points_2d, projection_info, reference_points_3d):
    ...
    # 计算参考点云的2D投影
    reference_points_2d,fa  = project_to_plane(reference_points_3d[:, :3], normal_vector)
...
if __name__ == '__main__':
    path = '../data/model.ply'
    path_density  = '../data/meshed-poisson.ply'
    save_path = '../data/srm2.ply'

    spase_points = read_ply_ascii(path)
    density_points = read_ply_ascii(path_density)

    # 获取投影信息
    spase_points_2d, density_points_2d, projection_info ,project_spase_2d, project_density_2d = to2dimension(spase_points, density_points)
    ...
    density_per_windows, interpolated_points_3d ,all_insert_indices = calculate_density_per_windows(
        spase_points_2d, project_density_2d, windows_num=10, threshold=5000,
